 ...
 * - focal_length
   ...
   - Denotes the camera's focal length specified using *35mm* film
     ...
     view using the :monosp:`fov` parameter. See the main description for
further details. (Default: :monosp:`50mm`)
 * - fov_axis
   - |string|
   - When the parameter :monosp:`fov` is given (and only then), this parameter
further specifies the image axis, to which it applies.
 ...
 * - principal_point_offset_x, principal_point_offset_y
   - |float|
   - Specifies the position of the camera's principal point relative to the
center of the film.

.. subfigstart::
.. subfigure:: ../../resources/data/docs/images/render/sensor_perspective.jpg
   :caption: The material test ball viewed through a perspective pinhole camera.
(:monosp:`fov=28`)
.. subfigure::
../../resources/data/docs/images/render/sensor_perspective_large_fov.jpg
   :caption: The material test ball viewed through a perspective pinhole camera.
(:monosp:`fov=40`)
.. subfigend::
...
class TextureSpaceCamera final : public ProjectiveCamera<Float, Spectrum> {
public:
    ...
    TextureSpaceCamera(const Properties &props) : Base(props) {
        ...
        for (auto &obj : objs) {
            ...
            if (shape) {
                Log(Warn, "TextureSpaceCamera using shape: %s",
                    obj.second->id());
    ...
    void update_camera_transforms() {
        m_camera_to_sample = perspective_projection(
            m_film->size(), m_film->crop_size(), m_film->crop_offset(), m_x_fov,
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        SurfaceInteraction3f pt = sampled_shape->eval_parameterization(
            position_sample + m_principal_point_offset, active);
    ...
    std::string to_string() const override {
        ...
        oss << "TextureSpaceCamera[" << std::endl
...
MTS_IMPLEMENT_CLASS_VARIANT(TextureSpaceCamera, ProjectiveCamera)
MTS_EXPORT_PLUGIN(TextureSpaceCamera, "Perspective Camera");
